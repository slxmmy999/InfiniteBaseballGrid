...
export default {
  ...
  async created () {
    const { data } = await this.$axios.get('http://192.168.1.242:5000/get_new_grid')
    this.teams = data
    EventBus.$on('player-selected', async () => {
      ...
      const data = await this.$axios.get(`http://192.168.1.242:5000/validate_player?name=${player}&team1=${team1}&team2=${team2}`)
